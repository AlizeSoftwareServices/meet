...
let EmailService = EmailService_1 = class EmailService {
    ...
    getBaseTemplate(title, contentHtml) {
        return `
      <!DOCTYPE html>
      <html>
      <head>
    ...
    async sendBookingConfirmation(email, guestName, eventTitle, startTime, meetLink, duration = 30, hostName = 'Host', hostEmail) {
    ...
    `;
        ...
        try {
            const icsPromise = new Promise((resolve, reject) => {
                ...
                (0, ics_1.createEvent)(event, (err, val) => (err ? reject(err) : resolve(val)));
            });
    ...
    async sendCancellationEmail(email, guestName, eventTitle, reason) {
        const content = `
      <p>Hello <strong>${guestName}</strong>,</p>
    ...
    async sendRescheduleEmail(email, guestName, eventTitle, newStartTime, meetLink) {
    ...
    async sendReminderEmail(email, guestName, eventTitle, startTime, meetLink) {
